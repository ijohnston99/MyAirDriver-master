module.exports = [
    {
        name: 'MyAir Current Temperature',
        deviceId: 9,
        getStg: [function(centralActualTemp){
    ...
    },
    {
        ...
        deviceId: 31,
        getStg: [function(centralDesiredTemp) {
		...
		setStg: [function(ipAddr, data, lastData) { // example:   curl -d '{"t_cool":67,"t_heat":67}' http://1.1.1.1/tstat
			if (isNaN(data)) {
				...
				if (lastData.t_heat) { return '{ "t_heat":' + data + '}'; }
				else if (lastData.t_cool) { return '{ "t_cool":' + data + '}'; }
...
//     },
    {
		...
		setStg: [function(ipAddr, data, lastData) { // example:   curl -d '{"tmode":1}' http://1.1.1.1/tstat  (turns to heat mode)
			...
			if (modNum[data]) { return '{ "tmode":' + modNum[data] + '}'; } else { return undefined; };
        }],
    },
    {
		...
		canSet: true,
		setStg: [function(ipAddr, data, lastData) { // example:   curl -d '{"fmode":1}' http://1.1.1.1/tstat  (turns to heat mode)
			var modNum = {"Thermostat Fan Auto Mode":0,"Thermostat Fan Auto/Circ":1,"Thermostat Fan Mode On":2};
			if (modNum[data]) { return '{ "fmode":' + modNum[data] + '}'; } else { return undefined; };
        }],
    };        
//     {
...
/*
    ...
    setStg: a function to call if device is writable upon write which will return a string of the data for a post request to the thermsostat, given ((string)ipAddr, (string)data), (json object)lastData as arguments
